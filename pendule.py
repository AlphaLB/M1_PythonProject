...
class pendule:
    ...
    def getqdd(self, Q=None, u=None):
        ...
        return np.array([[(u[0,0]+self.m*self.l*sin(q[0,0])-self.nu*qd[0,0])/(self.mu*self.l**2)]])
    ...
    def drawDoublePendulum(self, screen, color = (255, 255, 255), scale = 1., offset = np.zeros((2,), dtype = np.int_), q = None):
        """Dessine un double pendule"""
        pass
...
def simulationControl(aPend, Q0, QTarget, Kp,Kpd):
    ...
    fIntP = lambda t, Q:aPend.getQd(Q.reshape((2, 1)), u=np.dot(Kp,Q-QTarget))
    fIntPD = lambda t, Q:aPend.getQd(Q.reshape((2, 1)), u=np.dot(Kpd, Q-QTarget))
...
def simulationControlPID(aPend, Q0, QTarget, Kpd, Ki):
    ...
    # Function for integration -> Proportional
    fInt = lambda t, Q: np.vstack(( aPend.getQd(Q[:2].reshape((2, 1)), u=np.dot(Kpd, Q[:2].reshape((2, 1))-QTarget)+Ki*Q[2]), Q[0,0]-QTarget[0,0] ))
